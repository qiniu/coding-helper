...
配置应用后，Coding Helper 会先将已有的 `~/.codex/config.toml` 和 `~/.codex/model-catalogs/qnaigc.json` 备份为带时间戳的 `.bak-fenno-*` 文件，并在终端显示每个备份路径。随后会用七牛 Codex 配置完整覆盖 `config.toml`，避免与客户原配置冲突，并将 API Key 写入 Codex 自己的 `~/.codex/auth.json` 认证缓存。它不依赖 `QINIU_API_KEY` shell 环境变量。
