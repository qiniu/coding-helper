...
When Codex configuration is applied, Coding Helper first backs up an existing `~/.codex/config.toml` and `~/.codex/model-catalogs/qnaigc.json` to timestamped `.bak-fenno-*` files and prints every backup path. It then replaces `config.toml` with the Qiniu Codex configuration to avoid conflicts with existing settings, and stores the API Key in Codex's own `~/.codex/auth.json` auth cache. It does not rely on a `QINIU_API_KEY` shell environment variable.
